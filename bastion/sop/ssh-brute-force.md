...
## Response
1. Add the offending IP to the `bastion_block` ipset with a 1-hour timeout. A
   standing iptables rule drops all traffic matching the set, so SSH from that
   IP is refused for the next hour, after which ipset expires the entry
   automatically.
2. Seal an audit-trail entry: source IP, attempt count, window, severity, the
   SOP referenced, the model's reasoning, confidence, and the exact action
   taken.
3. Do NOT block whitelisted IPs under any circumstances — the monitoring probe
...
## Notes
Enforcement is automatic and time-boxed (1 hour). Repeat offenders re-trip the
rule on their next burst after the block expires.
